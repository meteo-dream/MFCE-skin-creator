...
func _ready() -> void:
	pass
	#gen_image_files("frog", "F:\\Godot\\Новая папка")

func copy_settings(suit: String, path: String) -> Error:
	return OK

func gen_all_suits(path: String) -> String:
	for i: String in TEXTURES.keys():
		var new_path: String = path.path_join(i)
		...
		var err: Error = gen_image_files(i, path.path_join(i))
		if err: return "Error creating %s suit: %s" % [i, error_string(err)]
	return "Success"

func gen_image_files(suit: String, path: String) -> Error:
	var img: Image = TEXTURES[suit].get_image()
	var _offsets: Dictionary
	match suit:
		"small": _offsets = OFFSETS_SMALL
		"super": _offsets = OFFSETS_SUPER
		"frog":  _offsets = OFFSETS_FROG
		_:       _offsets = OFFSETS_FULL
	for i: String in _offsets.keys():
		var offset: Array = _offsets[i]
		...
		var _path := path.path_join(i + ".png")
